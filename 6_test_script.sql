...
-- Verifies correctness of user creation, event creation, participation, sightings, querying,
-- and error handling (such as preventing deletion of users with participation).

SET SERVEROUTPUT ON;

DECLARE
    -- Variable declarations for capturing IDs of inserted data
    v_user1_id users.user_id%TYPE;
    v_user2_id users.user_id%TYPE;
...
BEGIN
    ...
    BEGIN
        -- Try to find a location named "Test Location"
        SELECT location_id INTO v_location_id FROM locations WHERE name = 'Test Location' AND ROWNUM = 1;
    EXCEPTION
        -- If not found, insert it
        WHEN NO_DATA_FOUND THEN
            INSERT INTO locations (location_id, name, region, latitude, longitude)
            VALUES ((SELECT COALESCE(MAX(location_id), 0) + 1 FROM locations), 'Test Location', 'Test Region', 48.15, 17.11)
            RETURNING location_id INTO v_location_id;
            COMMIT;
    END;

    DBMS_OUTPUT.PUT_LINE('=== Inserting users and capturing IDs ===');
    -- Insert User 1 (or fetch existing)
    BEGIN
        SELECT user_id INTO v_user1_id FROM users WHERE email = 'alice1@example.com' AND ROWNUM = 1;
    EXCEPTION
        WHEN NO_DATA_FOUND THEN
            INSERT INTO users (user_id, full_name, email, registration_date)
            VALUES ((SELECT COALESCE(MAX(user_id), 0) + 1 FROM users), 'Alice Smith', 'alice1@example.com', SYSDATE)
            RETURNING user_id INTO v_user1_id;
    END;

    -- Insert User 2 (or fetch existing)
    BEGIN
        SELECT user_id INTO v_user2_id FROM users WHERE email = 'bob1@example.com' AND ROWNUM = 1;
    EXCEPTION
        WHEN NO_DATA_FOUND THEN
            INSERT INTO users (user_id, full_name, email, registration_date)
            VALUES ((SELECT COALESCE(MAX(user_id), 0) + 1 FROM users), 'Bob Johnson', 'bob1@example.com', SYSDATE)
            RETURNING user_id INTO v_user2_id;
    END;

    DBMS_OUTPUT.PUT_LINE('=== Testing pkg_users.change_email ===');
    -- Update Alice's email only if target address doesn't already exist
    BEGIN
        SELECT COUNT(*) INTO v_count FROM users WHERE email = 'alice2@example.com';
        IF v_count = 0 THEN
    ...
    -- Insert Event 1
    BEGIN
        SELECT event_id INTO v_event1_id FROM events WHERE name = 'Spring Birdwatch' AND ROWNUM = 1;
    EXCEPTION
        WHEN NO_DATA_FOUND THEN
            INSERT INTO events (event_id, name, location_id, start_date, end_date)
            VALUES ((SELECT COALESCE(MAX(event_id), 0) + 1 FROM events), 'Spring Birdwatch', v_location_id, DATE '2025-05-01', DATE '2025-05-03')
            RETURNING event_id INTO v_event1_id;
    END;

    -- Insert Event 2
    BEGIN
        SELECT event_id INTO v_event2_id FROM events WHERE name = 'Summer Migration' AND ROWNUM = 1;
    EXCEPTION
        WHEN NO_DATA_FOUND THEN
            INSERT INTO events (event_id, name, location_id, start_date, end_date)
            VALUES ((SELECT COALESCE(MAX(event_id), 0) + 1 FROM events), 'Summer Migration', v_location_id, DATE '2025-06-01', DATE '2025-06-02')
    ...
    -- Insert Bird 1
    BEGIN
        SELECT bird_id INTO v_bird1_id FROM bird_species WHERE common_name = 'Sparrow' AND ROWNUM = 1;
    EXCEPTION
        WHEN NO_DATA_FOUND THEN
            INSERT INTO bird_species (bird_id, common_name, scientific_name)
            VALUES ((SELECT COALESCE(MAX(bird_id), 0) + 1 FROM bird_species), 'Sparrow', 'Passer domesticus')
            RETURNING bird_id INTO v_bird1_id;
    END;

    -- Insert Bird 2
    BEGIN
        SELECT bird_id INTO v_bird2_id FROM bird_species WHERE common_name = 'Blackbird' AND ROWNUM = 1;
    EXCEPTION
        WHEN NO_DATA_FOUND THEN
            INSERT INTO bird_species (bird_id, common_name, scientific_name)
            VALUES ((SELECT COALESCE(MAX(bird_id), 0) + 1 FROM bird_species), 'Blackbird', 'Turdus merula')
            RETURNING bird_id INTO v_bird2_id;
    END;

    DBMS_OUTPUT.PUT_LINE('=== Testing pkg_participation.register_user ===');
    -- Register Alice to Event 1 (only if not already registered)
    BEGIN
        SELECT 1 INTO v_count FROM participation WHERE user_id = v_user1_id AND event_id = v_event1_id;
    EXCEPTION
        WHEN NO_DATA_FOUND THEN
    ...
    BEGIN
        SELECT 1 INTO v_count FROM participation WHERE user_id = v_user2_id AND event_id = v_event1_id;
    EXCEPTION
        WHEN NO_DATA_FOUND THEN
    ...
    BEGIN
        SELECT 1 INTO v_count FROM participation WHERE user_id = v_user2_id AND event_id = v_event2_id;
    EXCEPTION
        WHEN NO_DATA_FOUND THEN
    ...
    -- Log bird sightings for both users in different events
    ...
    BEGIN
        -- Show the most commonly sighted bird name
    ...
    DECLARE
        v_duration NUMBER;
    BEGIN
        -- Print the duration in days of Event 1
    ...
    -- Unregister Bob from Event 2 (so he can be safely deleted later)
    ...
    BEGIN
        -- Try deleting Bob while still registered in Event 1 → should fail
